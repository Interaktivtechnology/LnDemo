...
define("USERNAME", "bcf@interaktiv.sg.bcf");
define("PASSWORD", "Interaktiv.1234");
define("SECURITY_TOKEN", "E5nYQaakO04tupaYeYaREnXt1");
ini_set("soap.wsdl_cache_enabled", 0);
require_once ('loader/soapclient/SforcePartnerClient.php');

try {
    $mySforceConnection = new SforcePartnerClient();
    $mySforceConnection->createConnection("loader/soapclient/partner.wsdl.xml");
...
function listInterest(){
    ...
    foreach($account->fields as $field){
        if($field->name == 'Volunteer_Area_of_Interests__c'){
            ...
            foreach($field->picklistValues as $value){
                if($value->label == 'Workplace programmes'){
