...
class MigrationIpc extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['Old_Serial_Number__c'], 'checkOldSerialNumber', 'message' => "Already Inserted"],
        ];
    }
    ...
    public function checkOldSerialNumber($attribute, $params)
    {

        if((int)$this->Old_Serial_Number__c != 0 && $this->isNewRecord):
            $row = self::find()->where(["Old_Serial_Number__c" => $this->Old_Serial_Number__c])->one();
            if($row)
                $this->addError($attribute, "Serial Number Duplicated");
        endif;
    }


    public function CreateLog($rowNumber, $ipcRow){
        $log = MigrationLog::find()->where(["DataSource" => "IPC", "RowNumber" => $rowNumber,
            "Object" => "Donation"])->one();
        $log = $log == null ? new MigrationLog() : $log;

        $log->Id = "ipc_"."d_".$rowNumber;
        $log->RowNumber = $rowNumber;
        $log->RowKey = $this->Old_Serial_Number__c;
        $log->Value = json_encode($ipcRow);
        $log->DataSource = "IPC";
        $log->Object = "Donation";
